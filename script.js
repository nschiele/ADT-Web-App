...
function isConsentGiven() {
  ...
  async function retrieveFromServer() {
    ...
    if (treeName == "flag") {
        alert("Hey! Don't try to hack us! Thank goodness we protected ourselves against this.\n\n" +
            "We now have a super difficult secret code set to get access to private information.\n\n" +
            "It must take a genius and some good serious research, as is done on https://www.w3schools.com/sql/sql_injection.asp, to crack this!"
        );
    } else if (treeName.includes("OR 1=1") || treeToken.includes("OR 1 = 1") || treeName.includes("'OR '1'='1") || treeToken.includes("' OR '1'='1") || treeName.includes('"OR 1 =1') || treeName.includes('"OR 1= 1')) {
        const epoch = Math.floor(Date.now());
        const connectionstatus = readyconnection + "_" + epoch + "}";
        alert("Wow! You actually managed to inject some SQL code to leak some important information from this database! You will be awarded for this!\n\n" +
            connectionstatus + "\n\n" +
            "On a serious matter, this is an example (not a real one) of a SQL injection vulnerability. These are very common on the web and can lead to large data leaks! Always make sure to sanitize the user inputs!");
    } else {
        try {
 ...
 *
 * Purpose:
 * This function traverses a tree-like structure of "nodes" (any object that exposes
 * a `root` with a `position()` method, and a `children` array) and expands four
 * global bounding variables (minX, minY, maxX, maxY) so that they describe an
 * axis-aligned bounding box that contains every node's visible area according to
 * a fixed width called `standardWidth`.
 *
 * At each node the function:
 *  - queries the node's root position (via `node.root.position()`),
 *  - computes the left edge as (x - standardWidth/2),
 *  - computes the right edge as (x + standardWidth/2),
 *  - treats the y position as the vertical coordinate for both top and bottom
 *    comparisons (the code assumes `position().y` is the vertical coordinate used
 *    for min/max calculations),
 *  - updates the global bounding values (minX, minY, maxX, maxY) if the
 *    computed edges extend beyond the current boundaries,
 *  - then recursively visits every child in `node.children`.
 *
 * Why this function exists:
 * When rendering, exporting, or taking screenshots of a graph/tree layout that
 * uses node-centered coordinates, one often needs to compute the total area that
 * must be captured. This function converts a set of center coordinates into the
 * minimum axis-aligned rectangle that encloses the whole layout — useful for
 * cropping render output, setting camera bounds, setting scroll/viewport limits,
 * or allocating an image buffer of the correct dimensions before a rasterization
 * step. The function focuses on a simple width-based notion of node extent:
 * nodes are considered to occupy a horizontal band of width `standardWidth`
 * centered at their x coordinate. The vertical extent is taken from the y
 * coordinate itself (i.e., nodes are treated as points vertically), which is a
 * valid simplification when nodes are rendered as circles/rectangles of fixed
 * height or when vertical extent is negligible or handled elsewhere.
 *
 * Important notes about the environment and side-effects:
 *  - The function mutates the variables `minX`, `minY`, `maxX`, `maxY`. They
 *    must exist in an enclosing scope (e.g., module-level or a closure) before
 *    the first call. Typical initialization code would set:
 *      minX = +Infinity;
 *      minY = +Infinity;
 *      maxX = -Infinity;
 *      maxY = -Infinity;
 *    Then call `screenshotWalk(root)` and afterwards use the four values to
 *    derive width/height for the screenshot or image canvas:
 *      width  = Math.max(0, Math.ceil(maxX - minX));
 *      height = Math.max(0, Math.ceil(maxY - minY));
 *
 *  - The function also expects `standardWidth` to be defined and numeric in the
 *    same scope. It is used symmetrically for left/right expansion, as
 *    (x - standardWidth/2) and (x + standardWidth/2). If nodes have varying
 *    widths, `standardWidth` should be replaced with a node-specific property,
 *    and the code adapted accordingly.
 *
 *  - Because this function relies on `node.root.position()` being present, if
 *    any node does not have `root` or `position()` or if `position()` doesn't
 *    return an object with numeric `.x` and `.y` properties, this function will
 *    throw. Defensive checks are omitted here for brevity but are recommended
 *    for production use (see "Robustness and defensive improvements" below).
 *
 *  - The function assumes that positions are in the same coordinate space for
 *    all nodes (i.e., no local transforms or different coordinate frames). If
 *    nodes exist in nested coordinate systems or have local transforms, you
 *    must first convert positions to a common root coordinate system (e.g.,
 *    by applying parent transforms) before using this walker, or adapt the
 *    walker to request world/global positions from the API.
 *
 * Walk strategy (recursion):
 *  - This implementation uses depth-first recursion. At each node we perform
 *    constant-time position checks and possibly update four global numbers,
 *    then recursively traverse children.
 *  - The order of visiting children does not matter for the resulting bounding
 *    box; it only matters for performance if children are lazily computed or
 *    expensive to access (rare).
 *  - Recursion depth equals the tree height. For very deep trees this can blow
 *    the JavaScript call stack. If your tree can be arbitrarily deep (e.g.,
 *    linked-list-like chains of nodes), convert the implementation to use an
 *    explicit stack/loop to avoid `Maximum call stack size exceeded` errors.
 *
 * Complexity:
 *  - Time complexity: O(N) where N is the total number of nodes visited. Each
 *    node yields a constant amount of work (position read + four comparisons +
 *    constant arithmetic).
 *  - Space complexity: O(h) call stack due to recursion, where h is the tree
 *    height. If the tree is balanced-ish, this is O(log N); if degenerate, O(N).
 *
 * Edge cases and behavior detail:
 *  - If `minX`/`minY`/`maxX`/`maxY` are not initialized appropriately before
 *    the first call, the logic will still work if they're set to +Infinity/-Infinity,
 *    but may produce incorrect results if they are left as `undefined` or other
 *    numeric values that do not represent "unbounded" initial conditions.
 *
 *  - If the tree is empty or the root node passed is `null`/`undefined`, the
 *    function will throw. If empty trees are legitimate inputs, guard before
 *    calling:
 *      if (root) screenshotWalk(root);
 *
 *  - If `standardWidth` is 0, each node contributes only its x coordinate to
 *    the bounding box; left and right edges become equal to x. A width < 0
 *    will invert the subtraction/addition semantics and may produce surprising
 *    results — make sure `standardWidth >= 0`.
 *
 *  - Nodes with non-finite coordinates (NaN, Infinity) will corrupt the global
 *    bounds. Sanitizing coordinates with `Number.isFinite()` checks is advised
 *    when input sources are untrusted.
 *
 * Robustness and defensive improvements (suggestions):
 *  - Validate input shape at the top of the function:
 *      if (!node || !node.root || typeof node.root.position !== 'function')
 *        return; // or throw an informative error
 *
 *  - Cache the result of `node.root.position()` to avoid repeating the call:
 *      const pos = node.root.position();
 *      const x = Number(pos && pos.x);
 *      const y = Number(pos && pos.y);
 *    This also provides a single place to coerce and validate numeric values.
 *
 *  - If nodes store dimensions (width/height) individually, use those instead
 *    of a single `standardWidth`:
 *      const halfWidth = (Number(node.width) || standardWidth) / 2;
 *      left = x - halfWidth;
 *      right = x + halfWidth;
 *
 *  - To support node height, update minY/maxY by subtracting/adding halfHeight:
 *      minY = Math.min(minY, y - halfHeight);
 *      maxY = Math.max(maxY, y + halfHeight);
 *    The current implementation treats the node's y as both top and bottom
 *    reference, which is fine only when vertical extent is irrelevant or handled
 *    elsewhere.
 *
 *  - Replace recursion with iteration for extremely deep trees:
 *      const stack = [node];
 *      while (stack.length) {
 *        const n = stack.pop();
 *        // compute pos, update bounds...
 *        for (const c of n.children) stack.push(c);
 *      }
 *    This uses an explicit stack and avoids call-stack overflow at the cost of
 *    O(h) heap memory for the stack.
 *
 * Concurrency / reentrancy considerations:
 *  - The function mutates global boundaries; therefore concurrent or
 *    interleaved invocations will collide. To compute bounds for multiple
 *    independent trees in parallel, either:
 *      - Provide a bounding object as a parameter to the function (recommended),
 *        so each traversal updates its own bounds object:
 *          screenshotWalk(node, bounds);
 *      - Or create a wrapper that initializes a fresh set of bounds before
 *        calling the walker and returns them after the traversal finishes.
 *
 *  - Example of a reentrant signature:
 *      function screenshotWalk(node, bounds = {minX: Infinity, minY: Infinity, maxX: -Infinity, maxY: -Infinity}, standardWidthOverride)
 *        // use bounds and standardWidthOverride inside; return bounds at the end
 *

 * Testing notes:
 *  - Unit tests should verify:
 *      * a single-node tree expands bounds by ±standardWidth/2 on X and sets Y correctly,
 *      * a two-node tree with nodes at different X/Y updates min/max appropriately,
 *      * empty children arrays do not affect correctness,
 *      * negative or zero standardWidth behaves as documented,
 *      * non-numeric positions are handled (if defensive checks are added).
 *
 * Example usage:
 *  // set up globals
 *  minX = Infinity; minY = Infinity; maxX = -Infinity; maxY = -Infinity;
 *  standardWidth = 50;
 *  // tree: { root: { position: () => ({x:10, y:20}) }, children: [...] }
 *  screenshotWalk(root);
 *  // after call: use the bounds to compute the screenshot size
 *  const width = Math.ceil(maxX - minX);
 *  const height = Math.ceil(maxY - minY);
 *
 * Security implications:
 *  - Because the function simply reads numeric values and mutates numeric
 *    variables, there are no direct security concerns. However, if the node
 *    objects are user-provided and their `position()` implementation executes
 *    arbitrary code (e.g., getters with side effects), calling this walker
 *    will execute that code. Treat untrusted node data cautiously.
 *
 * Style and readability:
 *  - The original implementation is terse and idiomatic for quick in-scope
 *    scripts. For improved clarity, prefer caching positions in local `const`
 *    variables and consider renaming `node.root.position()` to something shorter
 *    if it's called frequently within a tight loop.
 *
 * Summary of invariants guaranteed by this function (assuming sane inputs):
 *  - After traversal, minX <= x - standardWidth/2 and maxX >= x + standardWidth/2
 *    for every node's x coordinate encountered in the tree.
 *  - After traversal, minY <= y and maxY >= y for every node's y coordinate
 *    encountered in the tree.
 *
 * Final suggestion:
 *  - If you want a more robust and reusable utility, convert the function into
 *    a pure function that accepts both the root node and a configuration/bounds
 *    object and returns the computed bounds rather than mutating external
 *    variables. That reduces coupling and makes unit testing simpler.
 */
